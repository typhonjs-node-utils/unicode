import { inflateSync, deflateSync } from '#runtime/data/compress';
...
class ConstB {
     ...
     */
    static DATA_0800_OFFSET = this.NEW_DATA_START_OFFSET + 0x780;
    /**
     * Start with allocation of 16k data entries.
     */
    static INITIAL_DATA_LENGTH = 1 << 14;
    /**
     * Grow about 8x each time.
     */
    static MEDIUM_DATA_LENGTH = 1 << 17;
    /**
     ...
     */
    static MAX_DATA_LENGTH_RUNTIME = 0xffff << Const.INDEX_SHIFT;
    /**
     *
     */
    static INDEX_1_LENGTH = 0x110000 >> Const.SHIFT_1;
    /**
     * Maximum length of the build-time data array.
     * One entry per `0x110000` code points, plus the illegal-UTF-8 block and the null block, plus values for the `0x400`
     * surrogate code units.
     */
    static MAX_DATA_LENGTH_BUILDTIME = 0x110000 + 0x40 + 0x40 + 0x400;
    /**
     ...
     */
    static INDEX_GAP_OFFSET = Const.INDEX_2_BMP_LENGTH;
    static INDEX_GAP_LENGTH = ((Const.UTF8_2B_INDEX_2_LENGTH + Const.MAX_INDEX_1_LENGTH) +
     ...
     */
    static MAX_INDEX_2_LENGTH = (0x110000 >> Const.SHIFT_2) + Const.LSCP_INDEX_2_LENGTH +
        this.INDEX_GAP_LENGTH + Const.INDEX_2_BLOCK_LENGTH;
    /**
     * The null index-2 block, following the gap in the index-2 table.
     */
    static INDEX_2_NULL_OFFSET = this.INDEX_GAP_OFFSET + this.INDEX_GAP_LENGTH;
    /**
     * The start of allocated index-2 blocks.
     */
    static INDEX_2_START_OFFSET = this.INDEX_2_NULL_OFFSET + Const.INDEX_2_BLOCK_LENGTH;
    /**
     * Maximum length of the runtime index array.
     * Limited by its own 16-bit index values, and by uint16_t UTrie2Header.indexLength.
     * The actual maximum length is lower, (0x110000>>SHIFT_2)+UTF8_2B_INDEX_2_LENGTH+MAX_INDEX_1_LENGTH.
     */
    static MAX_INDEX_LENGTH = 0xffff;
}
...
class UnicodeTrieBuilder {
    ...
    constructor(initialValue, errorValue) {
        ...
        for (i = ConstB.DATA_NULL_OFFSET; i < ConstB.NEW_DATA_START_OFFSET; i++) {
            this.#data[i] = this.#initialValue;
        }
        ...
        for (j = j; j < 0xc0; j += Const.DATA_BLOCK_LENGTH) {
            this.#map[i++] = 0;
        }
        ...
        for (j = j; j < ConstB.NEW_DATA_START_OFFSET; j += Const.DATA_BLOCK_LENGTH) {
            this.#map[i++] = 0;
        }
        // set the remaining indexes in the BMP index-2 block
        // to the null data block
        for (i = 0x80 >> Const.SHIFT_2; i < Const.INDEX_2_BMP_LENGTH; i++) {
            this.#index2[i] = ConstB.DATA_NULL_OFFSET;
        }
        // Fill the index gap with impossible values so that compaction
        // does not overlap other index-2 blocks with the gap.
        for (i = 0; i < ConstB.INDEX_GAP_LENGTH; i++) {
            this.#index2[ConstB.INDEX_GAP_OFFSET + i] = -1;
        }
        ...
        for (i = i; i < ConstB.INDEX_1_LENGTH; i++) {
            this.#index1[i] = ConstB.INDEX_2_NULL_OFFSET;
        }
        // Preallocate and reset data for U+0080..U+07ff,
        // for 2-byte UTF-8 which will be compacted in 64-blocks
        // even if DATA_BLOCK_LENGTH is smaller.
        for (i = 0x80; i < 0x800; i += Const.DATA_BLOCK_LENGTH) {
    ...
    freeze() {
        ...
        if ((allIndexesLength > ConstB.MAX_INDEX_LENGTH) || // for unshifted indexLength
            ...
            ((dataMove + this.#dataLength) > ConstB.MAX_DATA_LENGTH_RUNTIME)) { // for shiftedDataLength
    ...
    setRange(start, end, value, overwrite) {
        ...
        if ((start & Const.DATA_MASK) !== 0) {
            ...
            if (nextStart <= limit) {
                this.#fillBlock(block, start & Const.DATA_MASK, Const.DATA_BLOCK_LENGTH, value, this.#initialValue, overwrite);
                start = nextStart;
            }
            else {
                this.#fillBlock(block, start & Const.DATA_MASK, limit & Const.DATA_MASK, value, this.#initialValue, overwrite);
                return this;
            }
        }
    ...
    #allocIndex2Block() {
        ...
        this.#index2.set(this.#index2.subarray(this.#index2NullOffset, this.#index2NullOffset + Const.INDEX_2_BLOCK_LENGTH), newBlock);
        return newBlock;
    }
    #compact() {
    ...
    #compactData() {
        ...
        while (i < this.#index2Length) {
            ...
            if (i === ConstB.INDEX_GAP_OFFSET) {
                i += ConstB.INDEX_GAP_LENGTH;
            }
        ...
        while ((newStart & (Const.DATA_GRANULARITY - 1)) !== 0) {
    ...
    #compactIndex2() {
        ...
        while (start < this.#index2Length) {
            ...
            while ((overlap > 0) && !this.#equalInt(this.#index2, (newStart - overlap), start, overlap)) {
                --overlap;
            }
            if ((overlap > 0) || (newStart < start)) {
                // some overlap, or just move the whole block
        ...
        for (i = 0; i < ConstB.INDEX_1_LENGTH; i++) {
            this.#index1[i] = this.#map[this.#index1[i] >> Const.SHIFT_1_2];
        }
    ...
    #findSameDataBlock(dataLength, otherBlock, blockLength) {
        ...
        while (block <= dataLength) {
            if (this.#equalInt(this.#data, block, otherBlock, blockLength)) {
                return block;
            }
    ...
    #findSameIndex2Block(index2Length, otherBlock) {
        ...
        for (let block = 0; block <= index2Length; block++) {
            if (this.#equalInt(this.#index2, block, otherBlock, Const.INDEX_2_BLOCK_LENGTH)) {
                return block;
            }
        }
        return -1;
    ...
    #getIndex2Block(c, forLSCP) {
        if ((c >= 0xd800) && (c < 0xdc00) && forLSCP) {
            return Const.LSCP_INDEX_2_OFFSET;
        }
...
export { UnicodeTrie, UnicodeTrieBuilder };
